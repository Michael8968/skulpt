var $builtinmodule = function(name)
{
    ...
    (function() {
      ...
      mod.create_sound = new Sk.builtin.func(function(sound_data, sample_rate, num_channels) {
          ...
          // Header
          var data = [
              0x52, 0x49, 0x46, 0x46,  // ChunkID: "RIFF" getByte(chunksize, 0), getByte(chunksize, 1), getByte(chunksize, 2), getByte(chunksize, 3),
      ...
      var canvas = function($gbl, $loc) {
          ...
          $loc.draw_text = new Sk.builtin.func(function(self, text, point, size, color, face) {
              ...
              self.context.fillText(text, Sk.builtin.asnum$(point.mp$subscript(0)), Sk.builtin.asnum$(point.mp$subscript(1)));
              return Sk.builtin.none.none$;
          });
          $loc.draw_point = new Sk.builtin.func(function(self, point, color) {
              ...
              self.context.fillStyle = Sk.ffi.unwrapo(color);
              self.context.fillRect(Sk.builtin.asnum$(point.mp$subscript(0)), Sk.builtin.asnum$(point.mp$subscript(1)), 1, 1);
              return Sk.builtin.none.none$;
          });
          $loc.draw_line = new Sk.builtin.func(function(self, pt1, pt2, width, color) {
              ...
              self.context.moveTo(Sk.builtin.asnum$(pt1.mp$subscript(0)), Sk.builtin.asnum$(pt1.mp$subscript(1))); self.context.lineTo(Sk.builtin.asnum$(pt2.mp$subscript(0)), Sk.builtin.asnum$(pt2.mp$subscript(1)));
              self.context.stroke();
              return Sk.builtin.none.none$;
          });
          $loc.draw_circle = new Sk.builtin.func(function(self, center, radius, linewidth, linecolor, fillcolor) {
              ...
              self.context.beginPath();
              self.context.arc(Sk.builtin.asnum$(center.mp$subscript(0)), Sk.builtin.asnum$(center.mp$subscript(1)), radius, 0, 2 * Math.PI, false);
              if (fillcolor !== Sk.builtin.none.none$) {
                  self.context.fill();
              };
              self.context.stroke();
          ...
          $loc.draw_polyline = new Sk.builtin.func(function(self, points, width, color) {
              ...
              self.context.moveTo(Sk.builtin.asnum$(point.mp$subscript(0)), Sk.builtin.asnum$(point.mp$subscript(1)));
              for (i=1; i<points.sq$length(); i++) {
                  point = points.mp$subscript(i);
                  if (!checkPoint(point)) {
                      throw new Sk.builtin.TypeError("each point in points must be a 2 element sequence");
                  };
                  self.context.lineTo(Sk.builtin.asnum$(point.mp$subscript(0)), Sk.builtin.asnum$(point.mp$subscript(1)));
              }
              self.context.stroke();
              return Sk.builtin.none.none$;
          });
          ...
          $loc.draw_image = new Sk.builtin.func(function(self, image, srcpos, srcdim, dstpos, dstdim, rot) {
              ...
              self.context.drawImage(im, srcx, srcy, srcw, srch, dstoffx, dstoffy, dstw, dsth);
      ...
      for (i=65; i<=90; i++) {
          ...
          mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap(ch), Sk.builtin.assk$(i, Sk.builtin.nmber.int$));
          code2key[i] = ch;
          // Uppercase
      ...
      // Add space
      mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap("space"), Sk.builtin.assk$(32, Sk.builtin.nmber.int$));
      code2key[32] = "space";
      // Add arrows
      mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap("left"), Sk.builtin.assk$(37, Sk.builtin.nmber.int$));
      mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap("up"), Sk.builtin.assk$(38, Sk.builtin.nmber.int$));
      mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap("right"), Sk.builtin.assk$(39, Sk.builtin.nmber.int$));
      mod.KEY_MAP.mp$ass_subscript(Sk.ffi.basicwrap("down"), Sk.builtin.assk$(40, Sk.builtin.nmber.int$));
      ...
      var frame = function($gbl, $loc) {
          $loc.__init__ = new Sk.builtin.func(function(self, title, canvas_width, canvas_height, control_width) {
              var framediv;
              // Save parameters
              self.title = title;
              ...
              self.frame_window = window.open('','myframe', 'width='+winwidth+',height='+winheight
                  +',location=0'
                  +',menubar=0'
                  +',toolbar=0'
                  +',directories=0'
              ...
              var framehtml = '<html>'+'<head>' + '<title>' + jstitle + '</title>' + '</head>' + '<body>' + '<div id=\"guiframe\"></div>' + '</body>' + '</html>';
                ...
                if (!self.keydown[evt.keyCode]) {
                    ...
                    if (self.keydownhandler) {
                        ...
                        try {
                            ...
                            Sk.misceval.callsim(self.keydownhandler, Sk.builtin.assk$(evt.keyCode, Sk.builtin.nmber.int$));
                        } catch (e) {
                            // Sk.error(e);
                            throw e;
                        };
          ...
          self.canvas.onkeyup = function(evt) {
              ...
              if (self.keyuphandler) {
                  ...
                  try {
                      ...
                      Sk.misceval.callsim(self.keyuphandler, Sk.builtin.assk$(evt.keyCode, Sk.builtin.nmber.int$));
                  } catch (e) {
                      throw e;
                  };
              };
          ...
          var mousemove = function (evt) {
              ...
              var pos = new Sk.builtins['tuple']([Sk.builtin.assk$(x, Sk.builtin.nmber.int$), Sk.builtin.assk$(y, Sk.builtin.nmber.int$)]);
          ...
          self.keyevents = statusBox("Key:", self.mouseevents.offsetHeight + 5, self.ctrlWidth);
          ...
          $loc.get_canvas_image = new Sk.builtin.func(function(self) {
            ...
            window.open(imagedata, "Canvas Image", "left=0,top=0,width=" + self.canvas.width + ",height=" + self.canvas.height + ",toolbar=0,resizeable=0");
            return Sk.builtin.none.none$;
          });
        ...
        // Python timer class
        var timer = function($gbl, $loc) {$loc.__init__ = new Sk.builtin.func(function(self, interval, handler) {
          // Initialize
      ...
      mod.create_timer = new Sk.builtin.func(function(interval, handler) {
        k.builtin.pyCheckArgs("create_timer", arguments, 2, 2);
