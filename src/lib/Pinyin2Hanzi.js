...
var $builtinmodule = function(name) {
  ...
  mod.DefaultDagParams = Sk.misceval.buildClass(
    mod,
    defaultDagParams_type_f,
    "DefaultDagParams",
    []
  );
...
function result_type_f($gbl, $loc) {
  ...
  $loc.path = Sk.misceval.callsimOrSuspend(
    Sk.builtins.property,
    path_getter,
    path_setter
  );
...
function dag_type_f($gbl, $loc) {
  ...
  $loc.__getitem__ = new Sk.builtin.func(function(self, k) {
    ...
    console.log("__getitem__", item);
    return item;
  });

  $loc.__len__ = new Sk.builtin.func(function (self) {
    var results = self["results"];
    var length = results ? results.v.length : 0;
    return Sk.ffi.remapToPy(length);
  })
}
