...
class Arrow(pygame.sprite.Sprite):
    def __init__(self, speed):
        ...
        imgP = []
        for i in range(4):
            aa = pygame.image.load('lessonA/jiantou/' + str(i+1)*3 + '.png').convert_alpha()
            imgP.append(aa)
        ...
        self.ys = random.choice(['Red','Blue','Purple'])
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if len(aList):
                ...
                ##########################
                game, score = logic(arrow, anjian, score, game)
                '''
                if arrow.ys == 'Gray':
                    if anjian == arrow.fx:
                        score += 1
